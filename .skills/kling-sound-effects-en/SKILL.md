...
This overseas package is configured from the supplied backend definition with four real API IDs. Server and generation-time operator calls use `https://<API_ID>@app-enx39clvtkht-api-ELbW03DAPQrY.gateway.appmedo.com/...` and `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` in the platform gateway-enabled runtime. Provider credentials stay in backend configuration, never in this package or the browser. Application user sessions remain separate from gateway credentials.
